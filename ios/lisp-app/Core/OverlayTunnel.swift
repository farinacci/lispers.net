...
enum OverlayTunnel {
    ...
    // Inject a RAW inner IP packet (NO LISP header) at the tunnel EID on 4341, riding the
    // utun — exactly what the PING app does. The LispTunnel extension captures it, strips
    // the outer IP+UDP, and hands the raw inner to its engine, which does the encap. Used
    // by the app's OWN Direct-ping tab when the xTR runs in the extension (Option B): the
    // app has no data socket then, so it injects like any overlay client instead.
    @discardableResult
    static func sendRawInner(_ inner: Data, toTunnelEID tunnelEID: UInt32, ifIndex: UInt32) -> Bool {
        let fd = socket(AF_INET, SOCK_DGRAM, 0)
        guard fd >= 0 else { return false }
        defer { close(fd) }
        ...
        addr.sin_family = sa_family_t(AF_INET)
        addr.sin_port = UInt16(4341).bigEndian
        addr.sin_addr.s_addr = tunnelEID.bigEndian
        let n = inner.withUnsafeBytes { raw in
            withUnsafePointer(to: &addr) {
                $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { sa in
                    sendto(fd, raw.baseAddress, raw.count, 0, sa,
                           socklen_t(MemoryLayout<sockaddr_in>.size))
