...
final class TunnelManager: ObservableObject {
    ...
    func start(eid: String, instanceID: Int) async {
        ...
        do {
            ...
            // Shown in Settings → General → VPN & Device Management → VPN. Make it clear
            // this VPN exists to run overlay apps with the LISP app (not general traffic).
            m.localizedDescription = "Overlay App VPN"
            m.isEnabled = true
    ...
    private func updateMirroring() {
        // Superseded by Option B (Phase 1): the extension now writes the REAL engine log
        // files (core/itr/etr) to the shared App-Group logs dir, and the app tails them
        // into the Logs tab via LispLog.loadTail (from the state mirror). The old
        // capture-file → itr-ring mirror only fed banners and would fight loadTail, so
        // it's disabled.
