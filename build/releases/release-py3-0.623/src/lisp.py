...
#
def lisp_on_aws():
    vm = getoutput("sudo dmidecode | grep -i amazon")
...
def lisp_etr_process_map_request(lisp_sockets, map_request, source, sport,
    ...
    if (map_request.rloc_probe and len(lisp_sockets) == 4):
        ...
        # RLOC-probe replies.
        #
        # If this is a data-plane encapsulated probe, the source will be the
        # translated address where NAT-info is stored. Try that first.
        #
        ...
        if (lisp_decent_nat and ir not in lisp_rtr_list):
            ...
            if (ni == None):
                #
                # For data-plane encapsulated probes, try NAT-info for the
                # actual source address (which may be the translated address).
                #
                ni = lisp_get_nat_info(source, None)
                if (ni != None):
                    #
                    # NAT-info found for translated source, so send reply
                    # to source address with the ephemeral port from NAT-info.
                    #
                    encap_dest = source
                #endif
            #endif
            if (ni == None):
                lprint("Could not find NAT-info state for {} or {}".format(
                    ir, source.print_address_no_iid()))
                return
            #endif
            ...
            #
            lisp_encap_rloc_probe(lisp_sockets, encap_dest, ni, packet)
            return
        #endif
    #endif
